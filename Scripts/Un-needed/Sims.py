...
for i in range(30):
    ...
    if i < 10:
        ...
        for j in range(n_sim):
            s,ext = model_sir(X0,mu,beta=b_mod*N_10_b[i,0],gamma=gamma + omega,tmax=tmax,tstep=tstep,rng=rng)
            sims.append(s)
            if ext == 0:
        ...
        for j in range(n_sim):
            s,ext = model_sir(X0,mu,beta=b_mod*N_100_b[i-10,0],gamma=gamma + omega,tmax=tmax,tstep=tstep,rng=rng)
            sims.append(s)
            if ext == 0:
                final_s.append(s*10)
            exts.append(ext)
        ...
        for j in range(n_sim):
            s,ext = model_sir(X0,mu,beta=b_mod*N_1000_b[i-20,0],gamma=gamma + omega,tmax=tmax,tstep=tstep,rng=rng)
            sims.append(s)
            if ext == 0:
                final_s.append(s)
            exts.append(ext)
...
np.savetxt(f"../Data/R0_{R0}/Sims_SIR/Summary/{sim_g}_g_Std{I0}s_{regime}_{R0}.csv",size_stds,delimiter=",")
